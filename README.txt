https://github.com/roguetech2/Kodi_credits
...
See \resources\example_skip.txt for how to build timestamp files. They should be in as start-stop, where start and stop are in hh:mm:ss format, such that with 0-1:30, the video would start at 1 minute 30 seconds. If the stop time is only seconds, then it skips forward that far, such that 1:30-15 would skip from 1:30 to 1:45. decimals are supported, but skips of less than a second are not recommended. If no end time is entered, it will skip to near the end to set the video as fully watched, then either stop or if a playlist, advance to the next video. Note that multiple time stamps files can be used for a single video - all potential matches as per the settings above will be combined. Timestamps can be set to apply to seasons and episodes by having a preceding line as, far instance, "s01" or "s01e01". Again, see example_skip.txt for more information.
